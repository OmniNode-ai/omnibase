...
#!/usr/bin/env python3
"""
Script to identify files that only have incorrect stamper changes.
...
This version only identifies files, does not revert them.
"""
...
def main():
    ...
    skip_files = {
        '.onextree', 
        'src/omnibase/model/model_node_metadata.py', 
        ...
        'identify_header_only_changes.py',
        'revert_incorrect_stamps.py',
        'revert_header_only_changes.sh'
    }
    
    for i, filepath in enumerate(modified_files):
        if i % 50 == 0:
            print(f"Processing {i+1}/{len(modified_files)} files...")
    ...
    print(f"\n=== SUMMARY ===")
    print(f"Total files processed: {len(modified_files)}")
    print(f"Files with only incorrect header changes: {len(files_to_revert)}")
    print(f"Files with content changes: {len(files_with_content)}")
    
    if files_to_revert:
        print(f"\nFiles that can be safely reverted (only incorrect headers):")
        for filepath in files_to_revert:
            print(f"  - {filepath}")
    
    print(f"\nTo revert the {len(files_to_revert)} files with only header issues:")
    print(f"python3 revert_incorrect_stamps.py")
