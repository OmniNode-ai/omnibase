...
entrypoint: markdown://stamping_engine
namespace: markdown://stamping_engine
meta_type: tool
...
- What is the best way to expose the stamping pipeline for CLI, API, and batch use?
- How should the engine handle legacy or malformed blocks, given that only canonical, protocol-compliant blocks are supported?
...
```python
def stamp_file(path: Path, content: str, handler: ProtocolFileTypeHandler) -> OnexResultModel:
    # 1. Extract block and rest
    prev_meta, rest = handler.extract_block(path, content)
    # 2. Compute hash, check dirty, update meta as needed
    # 3. Serialize block
...
## Protocol Interface Import Pattern
...
## See Also
- [ONEX Protocol Specification](../protocol/onex_protocol.md)
- [Node Metadata Model](../model/model_node_metadata.py)
- [Handler Implementations](../../src/omnibase/handlers/)
...
- **Testing:**
  - Ensure full coverage for all supported file types, edge cases, and error conditions.
  - Tests must be maintainable, isolated, and easy to extend for new handlers or protocols.
  - All idempotency and canonical normalization tests must pass before any milestone is considered complete.

- **Documentation:**
  - All design, usage, and extension points must be clearly documented for future maintainers.
