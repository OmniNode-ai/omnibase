...
- See [Testing Guide](../testing.md) and [Structured Testing](../structured_testing.md) for canonical patterns.
...
The stamper tool is included in the ONEX package and is installed automatically when you install the package:
...
```bash
poetry run onex stamp file /path/to/file.yaml
```
...
```bash
poetry run onex stamp file /path/to/file.yaml --write
```
...
```
Usage: onex stamp file [OPTIONS] PATH
...
Options:
  ...
  -a, --author TEXT      Author to include in stamp  [default: ONEX Team]
...
- `--author, -a <name>`: Author to include in the stamp (default: `ONEX Team`)
...
- Dry run (default):
  ```bash
  poetry run onex stamp directory .
  ```
- Actually write changes, recursively:
  ```bash
  poetry run onex stamp directory . --recursive --write
  ```
...
- Stamp only YAML files, excluding testdata (write mode, recursively):
  ```bash
  poetry run onex stamp directory . --include '**/*.yaml' --exclude 'tests/**' --recursive --write
  ```
- Stamp a single file:
  ```bash
  poetry run onex stamp file docs/README.md --author "developer"
  ```
