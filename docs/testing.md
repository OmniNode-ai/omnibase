...
### 1.3a Fixture Placement and Sharing Standard

- **Node-local fixtures** (used only by a single node's tests) must be placed in that node's `node_tests/fixtures/` directory.
- **Shared fixtures** (used by multiple nodes, protocol, or integration tests) must be placed in `src/omnibase/fixtures/` or, if runtime-specific, in `src/omnibase/runtimes/onex_runtime/v1_0_0/fixtures/`.
- **No fixture code may be duplicated**; all sharing must be via import from the canonical location.
- All test dependencies must be injected via fixtures, never hardcoded or duplicated.
- This ensures a single source of truth, reduces drift, and enables robust, registry-driven, protocol-pure testing across the ecosystem.
...
| integration  | Local service/container usage | `