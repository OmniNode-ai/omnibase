...
class LogEventTransformer(ast.NodeTransformer):
    ...
    def visit_Call(self, node):
        ...
        if (
            ...
            and node.func.id == 'emit_log_event'
        ) or (
            isinstance(node.func, ast.Attribute)
            and node.func.attr == 'emit_log_event'
        ):
            ...
            node.keywords.append(
                ast.keyword(arg='event_bus', value=ast.Attribute(value=ast.Name(id='self', ctx=ast.Load()), attr='_event_bus', ctx=ast.Load()))
            )
            self.changed = True
        return self.generic_visit(node)
