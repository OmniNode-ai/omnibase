...
from omnibase.nodes.template_node.v1_0_0.models.state import TemplateInputState, TemplateOutputState
from omnibase.nodes.tree_generator_node.v1_0_0.models.state import TreeGeneratorInputState, TreeGeneratorOutputState
...
class SchemaGeneratorNode(EventDrivenNodeMixin):
    ...
    def __init__(self, event_bus: Optional[ProtocolEventBus]=None, **kwargs):
        super().__init__(node_id='schema_generator_node', event_bus=
            event_bus, **kwargs)
        self.event_bus = event_bus or InMemoryEventBus()
        self.available_models: Dict[str, Type[BaseModel]] = {'stamper_input':
            StamperInputState, 'stamper_output': StamperOutputState,
            ...
            'template_input': TemplateInputState, 'template_output':
            TemplateOutputState, 'schema_generator_input':
            SchemaGeneratorInputState, 'schema_generator_output':
            SchemaGeneratorOutputState}

    def generate_schema(self, model_class: Type[BaseModel], output_path:
        Path, include_metadata: bool=True) ->None:
    ...
    def execute(self, input_state: SchemaGeneratorInputState, event_bus:
        Optional[ProtocolEventBus]=None, **kwargs
        ) ->SchemaGeneratorOutputState:
        """
        ...
        try:
            ...
            if input_state.models_to_generate:
                models_to_process = {name: model_class for name,
                    model_class in self.available_models.items() if name in
                    input_state.models_to_generate}
                invalid_models = set(input_state.models_to_generate) - set(self
                ...
                if invalid_models:
                    ...
                    return create_schema_generator_output_state(status=
                        ...
                        schemas_generated=[], output_directory=input_state.
                        output_directory, total_schemas=0, correlation_id=
            ...
            for model_name, model_class in models_to_process.items():
                ...
                try:
                    self.generate_schema(model_class, output_path,
            ...
            output_state = create_schema_generator_output_state(status=
                ...
                schemas_generated, output_directory=input_state.
                output_directory, total_schemas=len(schemas_generated),
                correlation_id=input_state.correlation_id)
...
def main() ->None:
    ...
    input_state = create_schema_generator_input_state(output_directory=args
        .output_directory, models_to_generate=args.models, include_metadata
...
def get_introspection() ->dict:
    ...
    return SchemaGeneratorNodeIntrospection.get_introspection_response(
        ).model_dump()
