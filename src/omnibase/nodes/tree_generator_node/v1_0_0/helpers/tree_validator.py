...
class OnextreeValidator(ProtocolOnextreeValidator):
    ...
    def validate_onextree_file(self, onextree_path: Path, root_directory: Path
        ...
        if not onextree_path.exists():
            errors.append(OnextreeValidationError(code=
                ValidationErrorCodeEnum.MISSING_FILE, message=
                f'.onextree file not found: {onextree_path}', path=str(
                onextree_path)))
            return OnextreeValidationResultModel(status=
                ValidationStatusEnum.ERROR, errors=errors, warnings=
                warnings, summary='.onextree file missing')
        if not root_directory.exists():
            errors.append(OnextreeValidationError(code=
                ValidationErrorCodeEnum.MISSING_FILE, message=
                f'Root directory not found: {root_directory}', path=str(
                root_directory)))
            return OnextreeValidationResultModel(status=
                ValidationStatusEnum.ERROR, errors=errors, warnings=
                warnings, summary='Root directory missing')
        try:
            ...
            self._validate_file_completeness(tree_model, actual_tree_model,
                errors)
            self._validate_file_types(tree_model, actual_tree_model, errors)
            status = (ValidationStatusEnum.SUCCESS if not errors else
                ValidationStatusEnum.ERROR)
            summary = ('Validation passed' if not errors else
                f'{len(errors)} error(s) found')
            return OnextreeValidationResultModel(status=status, errors=
                errors, warnings=warnings, summary=summary, tree=tree_model)
        except Exception as e:
            errors.append(OnextreeValidationError(code=
                ValidationErrorCodeEnum.UNKNOWN, message=
                f'Validation failed with exception: {str(e)}'))
            return OnextreeValidationResultModel(status=
                ValidationStatusEnum.ERROR, errors=errors, warnings=
                warnings, summary='Validation failed with exception')
    ...
    def _extract_all_file_paths(self, node: OnextreeTreeNode, current_path: str
        ) ->Set[str]:
        paths: Set[str] = set()
        node_path = (f'{current_path}/{node.name}' if current_path else
            node.name)
        paths.add(node_path)
    ...
    def _validate_tree_structure(self, tree: OnextreeTreeNode, actual_tree:
        OnextreeTreeNode, errors: List[OnextreeValidationError]) ->None:
        if tree.name != actual_tree.name:
            errors.append(OnextreeValidationError(code=
                ValidationErrorCodeEnum.STRUCTURE_MISMATCH, message=
                f'Root node name mismatch: {tree.name} vs {actual_tree.name}',
                path=tree.name))
        if tree.type != actual_tree.type:
            errors.append(OnextreeValidationError(code=
                ValidationErrorCodeEnum.STRUCTURE_MISMATCH, message=
                f'Root node type mismatch: {tree.type} vs {actual_tree.type}',
                path=tree.name))

    def _validate_file_completeness(self, tree: OnextreeTreeNode,
        actual_tree: OnextreeTreeNode, errors: List[OnextreeValidationError]
        ) ->None:
        ...
        for missing_file in missing_files:
            errors.append(OnextreeValidationError(code=
                ValidationErrorCodeEnum.MISSING_FILE, message=
                f'File exists in directory but missing from .onextree: {missing_file}'
                , path=missing_file))
        extra_files = tree_files - actual_files
        for extra_file in extra_files:
            errors.append(OnextreeValidationError(code=
                ValidationErrorCodeEnum.EXTRA_FILE, message=
                f'File exists in .onextree but not in directory: {extra_file}',
                path=extra_file))

    def _validate_file_types(self, tree: OnextreeTreeNode, actual_tree:
        OnextreeTreeNode, errors: List[OnextreeValidationError]) ->None:

        def compare_types(node1: OnextreeTreeNode, node2: OnextreeTreeNode,
            path: str) ->None:
            if node1.type != node2.type:
                errors.append(OnextreeValidationError(code=
                    ValidationErrorCodeEnum.TYPE_MISMATCH, message=
            ...
            if node1.type == 'directory' and node1.children and node2.children:
                ...
                for name in (set(children1) & set(children2)):
                    compare_types(children1[name], children2[name],
                        f'{path}/{name}')
