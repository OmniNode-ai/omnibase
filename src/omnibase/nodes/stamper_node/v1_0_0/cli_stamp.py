...
from omnibase.core.core_error_codes import get_exit_code_for_status as shared_get_exit_code_for_status
from omnibase.core.core_structured_logging import emit_log_event
from omnibase.enums import LogLevel, OnexStatus, OutputFormatEnum, TemplateTypeEnum
from omnibase.fixtures.mocks.dummy_schema_loader import DummySchemaLoader
from omnibase.protocol.protocol_stamper_engine import ProtocolStamperEngine
from omnibase.utils.directory_traverser import DirectoryTraverser, SchemaExclusionRegistry
...
def get_engine_from_env_or_flag(fixture: Optional[str]=None
    ...
    if fixture_path:
        return FixtureStamperEngine(Path(fixture_path), fixture_format=
            fixture_format)
    return StamperEngine(schema_loader=DummySchemaLoader(),
        directory_traverser=DirectoryTraverser(schema_exclusion_registry=
        schema_exclusion_registry), file_io=RealFileIO(), handler_registry=None, event_bus=_EVENT_BUS)


@app.command('file')
def file(paths: List[str]=typer.Argument(..., help=
    'Path(s) to file(s) to stamp'), author: str=typer.Option(
    'OmniNode Team', '--author', '-a', help='Author to include in stamp'),
    template_type_str: str=typer.Option('minimal', '--template', '-t', help
    ='Template type (minimal, full, etc.)'), overwrite: bool=typer.Option(
    False, '--overwrite', '-o', help='Overwrite existing metadata block'),
    repair: bool=typer.Option(False, '--repair', '-r', help=
    'Repair malformed metadata block'), output_fmt: OutputFormatEnum=typer.
    Option(OutputFormatEnum.TEXT, '--format', '-f', help=
    'Output format (text, json)'), fixture: Optional[str]=typer.Option(None,
    '--fixture', help=
    'Path to JSON or YAML fixture for protocol-driven testing')) ->int:
    """
    ...
    handler_registry = cast(Any, engine).handler_registry
    registered = handler_registry._handlers if hasattr(handler_registry,
        '_handlers') else {}
    ...
    emit_log_event(LogLevel.DEBUG,
        ...
        node_id=_COMPONENT_NAME, event_bus=_EVENT_BUS)
    emit_log_event(LogLevel.DEBUG,
        f'Stamper CLI received {len(paths)} files: {paths}', node_id=
        _COMPONENT_NAME, event_bus=_EVENT_BUS)
    emit_log_event(LogLevel.DEBUG,
    ...
    for path in paths:
        file_path = Path(path)
        emit_log_event(LogLevel.INFO, f'Processing file: {path}',
            node_id=_COMPONENT_NAME, event_bus=_EVENT_BUS)
        ...
        if handler is None:
            ...
            continue
        try:
            result = engine.stamp_file(file_path, template=template_type,
                overwrite=overwrite, repair=repair, author=author, event_bus=_EVENT_BUS)
            emit_log_event(LogLevel.INFO,
            ...
            emit_log_event(LogLevel.ERROR,
                f'Error processing file {path}: {e}', node_id=
                _COMPONENT_NAME, event_bus=_EVENT_BUS)
            any_error = True
            continue
        if output_fmt == OutputFormatEnum.JSON:
            typer.echo(json.dumps(result.model_dump(), indent=2, default=
                _json_default))
        else:
        ...
        if result.status == OnexStatus.ERROR:
            any_error = True
    emit_log_event(LogLevel.DEBUG,
        '[END] Stamper CLI finished processing all files.', node_id=
        _COMPONENT_NAME, event_bus=_EVENT_BUS)
    if any_error:
...
@app.command()
def directory(directory: str=typer.Argument(..., help=
    'Directory to process'), recursive: bool=typer.Option(False,
    '--recursive', '-r', is_flag=True, help=
    'Recursively process subdirectories (default: only top-level directory)'
    ), write: bool=typer.Option(False, '--write', '-w', is_flag=True, help=
    'Actually write changes to files (default: dry run)'), include:
    Optional[List[str]]=typer.Option(None, '--include', '-i', help=
    "File patterns to include (e.g., '*.yaml')"), exclude: Optional[List[
    str]]=typer.Option(None, '--exclude', '-e', help=
    'File patterns to exclude'), ignore_file: Optional[Path]=typer.Option(
    None, '--ignore-file', help='Path to .onexignore file'),
    template_type_str: str=typer.Option('minimal', '--template', '-t', help
    ='Template type (minimal, full, etc.)'), author: str=typer.Option(
    'OmniNode Team', '--author', '-a', help='Author to include in stamp'),
    overwrite: bool=typer.Option(False, '--overwrite', '-o', help=
    'Overwrite existing metadata blocks'), repair: bool=typer.Option(False,
    '--repair', help='Repair malformed metadata blocks'), force: bool=typer
    .Option(False, '--force', help=
    'Force overwrite of existing metadata blocks'), output_fmt:
    OutputFormatEnum=typer.Option(OutputFormatEnum.TEXT, '--format', '-f',
    help='Output format (text, json)'), fixture: Optional[str]=typer.Option
    (None, '--fixture', help=
    'Path to JSON or YAML fixture for protocol-driven testing'),
    discovery_source: str=typer.Option('filesystem', '--discovery-source',
    help=
    'File discovery source: filesystem, tree, hybrid_warn, hybrid_strict'),
    enforce_tree: bool=typer.Option(False, '--enforce-tree', help=
    'Error on drift between filesystem and .tree (alias for hybrid_strict)'
    ), tree_only: bool=typer.Option(False, '--tree-only', help=
    'Only process files listed in .tree (alias for tree)')) ->int:
    """
    ...
    try:
        ...
        emit_log_event(LogLevel.DEBUG,
            ...
            node_id=_COMPONENT_NAME, event_bus=_EVENT_BUS)
        result = engine.process_directory(Path(directory), template=
            template_type, recursive=recursive, dry_run=not write,
            include_patterns=include, exclude_patterns=exclude, ignore_file=
            ignore_file, author=author, overwrite=overwrite, repair=repair,
            force_overwrite=force, event_bus=_EVENT_BUS)
        if output_fmt == OutputFormatEnum.JSON:
            typer.echo(json.dumps(result.model_dump(), indent=2, default=
                _json_default))
        else:
            ...
            for msg in result.messages:
                prefix = ('[ERROR]' if msg.level == 'error' else '[WARNING]' if
                    msg.level == 'warning' else '[INFO]')
...
def main() ->None:
    try:
        ...
        import sys
        emit_log_event(LogLevel.ERROR, f'[STAMPER ERROR] {e}', node_id=
            _COMPONENT_NAME, event_bus=_EVENT_BUS)
        ...
        import traceback
        emit_log_event(LogLevel.ERROR,
            f'[FATAL] Unhandled exception in CLI: {e}', node_id=
            _COMPONENT_NAME, event_bus=_EVENT_BUS)
        typer.echo(f'[FATAL] Unhandled exception: {e}', err=True)
        typer.echo(traceback.format_exc(), err=True)
