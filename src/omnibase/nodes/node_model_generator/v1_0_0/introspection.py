...
# namespace: python://omnibase.nodes.node_model_generator.v1_0_0.introspection
# owner: OmniNode Team
...
"""
Introspection implementation for model_generator node.
...
from .error_codes import NodeModelGeneratorErrorCode
from .models.model_generator_state import NodeModelGeneratorInputState, NodeModelGeneratorOutputState


class NodeMetadataLoader:
    def __init__(self, node_directory: Path):
        self.node_directory = node_directory
        self.metadata_path = node_directory / "node.onex.yaml"
        self._metadata = None
        self._load_metadata()
...
class NodeModelGeneratorIntrospection(NodeIntrospectionMixin):
    """
    Canonical introspection class for the node_model_generator node.
    The metadata loader must be injected as a class attribute before use:
        NodeModelGeneratorIntrospection.metadata_loader = NodeMetadataLoader(node_directory=...)
    ...
    def get_metadata_loader(cls):
        if cls.metadata_loader is None:
            raise RuntimeError("NodeModelGeneratorIntrospection.metadata_loader must be injected before use.")
    ...
    def get_error_codes_class(cls):
        return NodeModelGeneratorErrorCode
    ...
    def get_cli_required_args(cls) -> List[CLIArgumentModel]:
        return [
            CLIArgumentModel(
                ...
                type="str",
                required=True,
                description="Required input field for node_model_generator",
                default=None,
                choices=None,
            ),
        ]
    ...
    def get_cli_optional_args(cls) -> List[CLIArgumentModel]:
        ...
        model_generator_args = [
            CLIArgumentModel(
                name="--optional-field",
                type="str",
                required=False,
                description="Optional input field for node_model_generator",
                default=None,
                choices=None,
            ),
            CLIArgumentModel(
        ...
        return base_args + model_generator_args
