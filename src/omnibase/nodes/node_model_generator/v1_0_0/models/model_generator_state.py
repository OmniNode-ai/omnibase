...
# To regenerate: poetry run onex run schema_generator_node --args='["src/omnibase/nodes/node_model_generator/v1_0_0/contract.yaml", "src/omnibase/nodes/node_model_generator/v1_0_0/models/state.py"]'
...
class ModelGeneratorOutputField(BaseModel):
    result: str
    details: Optional[str]

class NodeModelGeneratorInputState(BaseModel):
    version: SemVerModel  # Schema version for input state
    input_field: str  # Required input field for node_model_generator
    optional_field: OnexFieldModel = None  # Optional input field for node_model_generator
    ...
    contract_path: Optional[Path] = None  # Path to contract.yaml for model generation
...
class NodeModelGeneratorOutputState(BaseModel):
    ...
    output_field: ModelGeneratorOutputField = None  # 
    event_id: Optional[str] = None  # Unique event identifier (UUID)
    correlation_id: Optional[str] = None  # Correlation ID for tracing requests/events
    node_name: Optional[str] = None  # Name of the node processing the event
    node_version: Optional[SemVerModel] = None  # Version of the node
