...
from .introspection import NodeModelGeneratorIntrospection
from .models.model_generator_state import NodeModelGeneratorInputState, NodeModelGeneratorOutputState, ModelGeneratorOutputField
from omnibase.nodes.node_model_generator.protocols.input_validation_tool_protocol import InputValidationToolProtocol
from omnibase.nodes.node_model_generator.protocols.output_field_tool_protocol import OutputFieldTool as OutputFieldToolProtocol
...
from .registry.registry_model_generator import RegistryModelGeneratorNode
from .tools.tool_contract_to_model import ContractToModelTool
...
class NodeModelGenerator(MixinNodeIdFromContract, MixinIntrospectFromContract, NodeModelGeneratorIntrospection, ProtocolReducer):
    """
    ...
    **ALL business logic must be delegated to protocol-typed helpers/tools.**
    ...
    def __init__(
        ...
        if input_validation_tool is None:
            input_validation_tool = ToolInputValidation(
                NodeModelGeneratorInputState, NodeModelGeneratorOutputState, OnexFieldModel, node_id="node_model_generator"
            )
        ...
        if is_trace_mode():
            emit_log_event_sync(
                LogLevelEnum.TRACE,
                f"NodeModelGenerator instantiated",
    ...
    def run(self, input_state: NodeModelGeneratorInputState) -> NodeModelGeneratorOutputState:
        """
        Orchestrates scenario execution for direct invocation. Accepts a validated NodeModelGeneratorInputState model.
        ...
        contract_path = getattr(input_state, 'contract_path', None)
        if contract_path:
            tool = ContractToModelTool()
            models = tool.generate_models_from_contract(contract_path)
            for name, code in models.items():
                print(f"\n# Model: {name}\n{code}")
            # Return a dummy output state for now
            return NodeModelGeneratorOutputState(
                version=input_state.version,
                status=OnexStatus.SUCCESS,
                message="NodeModelGenerator ran successfully.",
                output_field=ModelGeneratorOutputField(result="contract_to_model executed", details=None),
        ...
        # Fallback: legacy/canonical output for scenarios without contract_path
        output_field = self.output_field_tool(input_state, input_state.model_dump())
        if isinstance(output_field, dict):
            output_field = ModelGeneratorOutputField(**output_field)
        elif not isinstance(output_field, ModelGeneratorOutputField):
            output_field = ModelGeneratorOutputField(result=str(output_field), details=None)
        return NodeModelGeneratorOutputState(
            version=input_state.version,
            status=OnexStatus.SUCCESS,
            message="NodeModelGenerator ran successfully.",
    ...
    def initial_state(self) -> StateModel:
        ...
        """
        return NodeModelGeneratorInputState(
            version=SemVerModel(
    ...
    def dispatch(self, state: StateModel, action: ActionModel) -> StateModel:
        """
        Intentionally minimal for the model_generator node: returns the state unchanged.
...
def main(event_bus=None):
    ...
    registry_model_generator_node.register_tool(BACKEND_SELECTION_KEY, StubBackendSelection)
    registry_model_generator_node.register_tool(INPUT_VALIDATION_KEY, ToolInputValidation)
    registry_model_generator_node.register_tool(OUTPUT_FIELD_KEY, tool_compute_output_field)
    # Add other tools as needed (bootstrap, health_check, etc.)
    tool_backend_selection = StubBackendSelection(registry_model_generator_node)
    input_validation_tool = ToolInputValidation(
    ...
    node = NodeModelGenerator(
        ...
        registry=registry_model_generator_node,
    )
...
def get_introspection() -> dict:
    """Get introspection data for the model_generator node."""
    return NodeModelGeneratorIntrospection.get_introspection_response()
