...
class RegistryEngine:
    ...
    def __init__(self, handler_registry: Optional[FileTypeHandlerRegistry]=None, event_bus=None) -> None:
        """
        ...
        if event_bus is None:
            raise RuntimeError('RegistryEngine requires an explicit event_bus argument (protocol purity)')
    ...
    def load_registry(self, input_state: RegistryLoaderInputState
        ...
        try:
            ...
            onextree_path = self._resolve_onextree_path(root_path,
                input_state.onextree_path)
            if not onextree_path or not onextree_path.exists():
                return RegistryLoaderOutputState(version=input_state.
                    version, status=OnexStatus.ERROR, message=
                    f"Failed to find .onextree file. Expected at {onextree_path or root_path / '.onextree'}"
                    , root_directory=str(root_path), onextree_path=str(
                    onextree_path) if onextree_path else None,
                    scan_duration_ms=(time.time() - start_time) * 1000)
            onextree_model = self._load_onextree_model(onextree_path)
            if not onextree_model:
                return RegistryLoaderOutputState(version=input_state.
                    version, status=OnexStatus.ERROR, message=
                    f'Failed to load .onextree from {onextree_path}',
                    root_directory=str(root_path), onextree_path=str(
                    onextree_path), scan_duration_ms=(time.time() -
                    start_time) * 1000)
            artifacts = self._discover_artifacts_from_onextree_model(
                onextree_model, root_path, input_state.artifact_types,
                input_state.include_wip)
            artifact_types_found = list(set(artifact.artifact_type for
                artifact in artifacts))
            scan_duration = (time.time() - start_time) * 1000
            valid_artifacts = [a for a in artifacts if a.metadata.get(
                '_is_valid', True)]
            invalid_artifacts = [a for a in artifacts if not a.metadata.get
                ('_is_valid', True)]
            ...
            message = (
                f'Successfully loaded {total_count} artifacts from .onextree')
            if self.errors:
                ...
                if fatal_errors:
                    ...
                    message = (
                        f'Loaded {total_count} artifacts with {len(non_fatal_errors)} warnings'
                        )
            if total_count > 0:
                details = []
                if valid_count > 0:
            ...
            return RegistryLoaderOutputState(version=input_state.version,
                status=status, message=message, artifacts=artifacts,
                artifact_count=total_count, valid_artifact_count=
                valid_count, invalid_artifact_count=invalid_count,
                wip_artifact_count=wip_count, artifact_types_found=
                artifact_types_found, root_directory=str(root_path),
            ...
            return RegistryLoaderOutputState(version=input_state.version,
                status=OnexStatus.ERROR, message=
                f'Registry loading failed: {str(e)}', root_directory=
                input_state.root_directory, scan_duration_ms=(time.time() -
                start_time) * 1000)

    def _resolve_onextree_path(self, root_path: Path, onextree_path:
        ...
        if onextree_path:
            ...
            if path.is_absolute():
                ...
                return root_path / path if (root_path / path).exists(
                    ) else None
    ...
    def _load_onextree_model(self, onextree_path: Path) ->Optional[OnextreeRoot
        ...
        try:
            if not onextree_path.exists():
                self.errors.append(RegistryLoadingError(path=str(
                    onextree_path), error_type=RegistryLoadingErrorTypeEnum
                    .MISSING_FILE, error_message=
                    f'Onextree file not found: {onextree_path}', is_fatal=True)
                    )
                return None
            onextree_model = OnextreeRoot.from_yaml_file(str(onextree_path))
            return onextree_model
        except Exception as e:
            self.errors.append(RegistryLoadingError(path=str(onextree_path),
                error_type=RegistryLoadingErrorTypeEnum.PARSE_ERROR,
                error_message=f'Failed to parse onextree: {str(e)}',
                is_fatal=True))
            return None

    def _discover_artifacts_from_onextree_model(self, onextree_model:
        OnextreeRoot, root_path: Path, artifact_types_filter: Optional[List
        [ArtifactTypeEnum]], include_wip: bool) ->List[RegistryArtifact]:
        """
        ...
        """
        artifacts = []
        all_artifact_types = [ArtifactTypeEnum.NODES, ArtifactTypeEnum.
            CLI_TOOLS, ArtifactTypeEnum.RUNTIMES, ArtifactTypeEnum.ADAPTERS,
            ArtifactTypeEnum.CONTRACTS, ArtifactTypeEnum.PACKAGES]
        artifact_types_to_process = (artifact_types_filter if
            artifact_types_filter else all_artifact_types)
        versioned_artifacts = onextree_model.find_versioned_artifacts()
        type_mapping = {'nodes': ArtifactTypeEnum.NODES, 'cli_tools':
            ArtifactTypeEnum.CLI_TOOLS, 'runtimes': ArtifactTypeEnum.
            RUNTIMES, 'adapters': ArtifactTypeEnum.ADAPTERS, 'contracts':
            ArtifactTypeEnum.CONTRACTS, 'packages': ArtifactTypeEnum.PACKAGES}
        for artifact_type_str, artifact_name, version, node in versioned_artifacts:
            ...
            if artifact_type and artifact_type in artifact_types_to_process:
                metadata_file = self._find_metadata_file_in_node(node,
                    artifact_type)
                if metadata_file:
                    full_path = Path(onextree_model.name
                        ) / artifact_type_str / artifact_name / version
                    artifact = self._create_artifact_from_onextree(
                        artifact_name, artifact_type, version, str(
                        full_path), metadata_file, root_path, include_wip)
                    if artifact:
    ...
    def _find_metadata_file_in_node(self, node: OnextreeNode, artifact_type:
        ...
        """
        metadata_files = {ArtifactTypeEnum.NODES: 'node.onex.yaml',
            ArtifactTypeEnum.CLI_TOOLS: 'cli_tool.yaml', ArtifactTypeEnum.
            RUNTIMES: 'runtime.yaml', ArtifactTypeEnum.ADAPTERS:
            'adapter.yaml', ArtifactTypeEnum.CONTRACTS: 'contract.yaml',
    ...
    def _create_artifact_from_onextree(self, name: str, artifact_type:
        ArtifactTypeEnum, version: str, path: str, metadata_file: str,
        root_path: Path, include_wip: bool) ->Optional[RegistryArtifact]:
        """
        ...
        try:
            ...
            return RegistryArtifact(name=name, version=version,
                artifact_type=artifact_type, path=str(artifact_path),
                metadata=metadata, is_wip=is_wip)
        except Exception as e:
            self.errors.append(RegistryLoadingError(path=
                f'{name}/{version}', error_type=
                RegistryLoadingErrorTypeEnum.LOAD_ERROR, error_message=
                f'Failed to load artifact: {str(e)}', is_fatal=False))
            return None

    def _load_artifact_metadata_with_validation(self, artifact_path: Path,
        metadata_file: str, is_wip: bool) ->tuple[Optional[Dict[str, Any]],
        bool]:
        """
        ...
        if not metadata_path.exists():
            if is_wip:
                ...
                self.errors.append(RegistryLoadingError(path=str(
                    metadata_path), error_type=RegistryLoadingErrorTypeEnum
                    .MISSING_METADATA, error_message=
                    f'Missing metadata file: {metadata_file}', is_fatal=False))
                return None, False
        try:
            ...
            if not isinstance(metadata, dict):
                self.errors.append(RegistryLoadingError(path=str(
                    metadata_path), error_type=RegistryLoadingErrorTypeEnum
                    .INVALID_METADATA, error_message=
                    'Metadata file is not a valid YAML dictionary',
                    is_fatal=False))
                return None, False
            ...
            self.errors.append(RegistryLoadingError(path=str(metadata_path),
                error_type=RegistryLoadingErrorTypeEnum.PARSE_ERROR,
