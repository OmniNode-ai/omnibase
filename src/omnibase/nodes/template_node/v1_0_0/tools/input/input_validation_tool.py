...
from omnibase.nodes.template_node.protocols.input_validation_tool_protocol import (
    ...
    def validate_input_state(
        ...
        emit_log_event_sync(
            ...
            context=make_log_context(
                node_id="template_node", correlation_id=correlation_id
            ),
        )
        try:
            emit_log_event_sync(
                LogLevelEnum.DEBUG,
                f"About to instantiate TemplateNodeInputState with: {input_state}",
                make_log_context(
                    node_id="template_node", correlation_id=correlation_id
                ),
            )
