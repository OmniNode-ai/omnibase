...
def generate_introspection(contract_path: Path, output_path: Path, contract: dict, contract_hash: str):
    ...
    Args:
        contract_path: Path to the contract.yaml file
        output_path: Path to write the generated state.py file (used to find node dir)
        contract: Parsed contract dict
        contract_hash: Canonical hash of contract.yaml
    """
    node_dir = output_path.parent.parent
    introspection_path = node_dir / "introspection.py"
    header = f"""# AUTO-GENERATED FILE. DO NOT EDIT.\n# Generated from contract.yaml\n# contract_hash: {contract_hash}\n# To regenerate: poetry run python src/omnibase/runtimes/onex_runtime/v1_0_0/codegen/contract_to_model.py {contract_path} {output_path}\n"""
    code = f"""{header}
...
def main():
    if len(sys.argv) < 3:
        emit_log_event_sync(
            LogLevelEnum.ERROR,
            "Usage: python -m omnibase.nodes.node_manager.v1_0_0.tools.tool_contract_to_model <contract.yaml> <output_state.py>",
            node_id="tool_contract_to_model"
        )
        sys.exit(1)
    ...
    emit_log_event_sync(
        LogLevelEnum.ERROR,
        "ERROR: This script requires proper dependency injection. Use the ToolContractToModel class with a logger_tool instead of calling this script directly. Example: tool = ToolContractToModel(logger_tool=your_logger); tool.generate_state_models(contract_path, output_path)",
        node_id="tool_contract_to_model"
    )
    sys.exit(1)
