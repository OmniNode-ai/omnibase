...
class DispatchActionModel(BaseModel):
    ...
    params: List[DispatchParamModel] = Field(
        default_factory=list, description="List of parameters for the action"
    )
    returns: DispatchReturnModel = Field(
        ..., description="Return value model for the action"
    )


class DispatchTableModel(BaseModel):
    actions: List[DispatchActionModel] = Field(
        default_factory=list, description="List of available dispatch actions"
    )
