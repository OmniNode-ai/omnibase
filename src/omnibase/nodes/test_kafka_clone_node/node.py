"""
TestKafkaClone Node (ONEX Canonical)
...
from .introspection import {NODE_CLASS}Introspection
from .models.state import {NODE_CLASS}InputState, {NODE_CLASS}OutputState, Model{NODE_CLASS}OutputField
from omnibase.nodes.{NODE_NAME}.protocols.input_validation_tool_protocol import InputValidationToolProtocol
from omnibase.nodes.{NODE_NAME}.protocols.output_field_tool_protocol import OutputFieldTool as OutputFieldToolProtocol
...
from omnibase.constants import (
    ...
    BOOTSTRAP_KEY,
    HEALTH_CHECK_KEY,
    NODE_TEST_KAFKA_CLONE_ID,
    EVENT_ID_KEY,
    TIMESTAMP_KEY,
    ONEX_TRACE_ENV_KEY,
)
from .registry.registry_{NODE_NAME} import Registry{NODE_CLASS}
...
class {NODE_CLASS}(MixinNodeIdFromContract, MixinIntrospectFromContract, {NODE_CLASS}Introspection, ProtocolReducer):
    ...
    **ALL business logic must be delegated to protocol-typed helpers/tools.**
    ...
    def __init__(
        ...
        # [ONEX_PROMPT] Customize DI and registry-driven tool resolution for your node here.
        ...
        if input_validation_tool is None:
            input_validation_tool = ToolInputValidation(
                {NODE_CLASS}InputState, {NODE_CLASS}OutputState, OnexFieldModel, node_id="{NODE_NAME}"
            )
        ...
        if is_trace_mode():
            emit_log_event_sync(
                LogLevelEnum.TRACE,
                f"TestKafkaCloneNode instantiated",
                context=make_log_context(node_id=self.node_id),
            )

    def handle_event(self, event: OnexEvent):
        """
        [ONEX_PROMPT] Implement event-driven scenario execution logic for {NODE_NAME} if needed. Delegate all business logic to protocol-typed helpers/tools.
    ...
    def run(self, input_state: {NODE_CLASS}InputState) -> {NODE_CLASS}OutputState:
        """
        [ONEX_PROMPT] Implement the main reducer logic for {NODE_NAME}. Delegate all business logic to protocol-typed helpers/tools.
        Orchestrates scenario execution for direct invocation. Accepts a validated TestKafkaCloneNodeInputState model.
        ...
        # Use protocol-compliant output field tool, but always wrap in ModelTestKafkaCloneOutputField
        output_field_kwargs = self.output_field_tool(input_state, input_state.model_dump())
        if isinstance(output_field_kwargs, dict):
            output_field = Model{NODE_CLASS}OutputField(**output_field_kwargs)
        elif isinstance(output_field_kwargs, Model{NODE_CLASS}OutputField):
            output_field = output_field_kwargs
        else:
            output_field = Model{NODE_CLASS}OutputField(result=str(output_field_kwargs))
        ...
        return {NODE_CLASS}OutputState(
            version=semver,
            status=OnexStatus.SUCCESS,
            message="{NODE_CLASS} ran successfully.",
    ...
    def bind(self, *args, **kwargs):
        """
        [ONEX_PROMPT] Implement bind logic for {NODE_NAME} if needed. This is typically used for event-driven or async nodes.
    ...
    def initial_state(self) -> StateModel:
        """
        [ONEX_PROMPT] Implement initial state logic for {NODE_NAME} if needed.
        Return the initial state for the reducer. Override as needed.
        """
        return {NODE_CLASS}InputState(
            version=SemVerModel(
    ...
    def dispatch(self, state: StateModel, action: ActionModel) -> StateModel:
        """
        [ONEX_PROMPT] Implement dispatch logic for {NODE_NAME} if needed.
        Intentionally minimal for the test_kafka_clone node: returns the state unchanged.
    ...
    def introspect(self):
        """
        [ONEX_PROMPT] Implement introspection logic for {NODE_NAME} if needed. This should expose node metadata, contract, and scenario registry.
...
def main(event_bus=None):
    ...
    config = ModelEventBusConfig.default()
    registry_{NODE_NAME} = Registry{NODE_CLASS}()
    # Register canonical tools (stub backend selection, input validation, output field, etc.)
    registry_{NODE_NAME}.register_tool(BACKEND_SELECTION_KEY, StubBackendSelection)
    registry_{NODE_NAME}.register_tool(INPUT_VALIDATION_KEY, ToolInputValidation)
    registry_{NODE_NAME}.register_tool(OUTPUT_FIELD_KEY, tool_compute_output_field)
    # Add other tools as needed (bootstrap, health_check, etc.)
    tool_backend_selection = StubBackendSelection(registry_{NODE_NAME})
    input_validation_tool = ToolInputValidation(
    ...
    node = {NODE_CLASS}(
        ...
        registry=registry_{NODE_NAME},
    )
    parser = argparse.ArgumentParser(description="{NODE_CLASS} CLI")
...
def get_introspection() -> dict:
    """Get introspection data for the {NODE_NAME} node."""
    return {NODE_CLASS}Introspection.get_introspection_response()
