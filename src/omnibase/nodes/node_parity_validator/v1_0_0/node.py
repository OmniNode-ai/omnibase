...
from .introspection import NodeParityValidatorIntrospection
from .models.state import ParityValidatorInputState, ParityValidatorOutputState, ModelParityValidatorOutputField
from omnibase.nodes.node_parity_validator.protocols.input_validation_tool_protocol import InputValidationToolProtocol
from omnibase.nodes.node_parity_validator.protocols.output_field_tool_protocol import OutputFieldTool as OutputFieldToolProtocol
...
from omnibase.nodes.node_parity_validator.protocols.protocol_cli_node_parity import ProtocolCliNodeParity
from omnibase.nodes.node_parity_validator.protocols.protocol_schema_conformance import ProtocolSchemaConformance
from omnibase.nodes.node_parity_validator.protocols.protocol_error_code_usage import ProtocolErrorCodeUsage
from omnibase.nodes.node_parity_validator.protocols.protocol_contract_compliance import ProtocolContractCompliance
from omnibase.nodes.node_parity_validator.protocols.protocol_introspection_validity import ProtocolIntrospectionValidity
...
class NodeParityValidator(MixinNodeIdFromContract, MixinIntrospectFromContract, NodeParityValidatorIntrospection, ProtocolReducer):
    """
    ...
    **ALL business logic must be delegated to protocol-typed helpers/tools.**
    ...
    def __init__(
        ...
        tool_cli_node_parity: ProtocolCliNodeParity = None,
        tool_schema_conformance: ProtocolSchemaConformance = None,
        tool_error_code_usage: ProtocolErrorCodeUsage = None,
        tool_contract_compliance: ProtocolContractCompliance = None,
        tool_introspection_validity: ProtocolIntrospectionValidity = None,
        ...
        if registry is not None:
            ...
            if hasattr(registry, 'get_tool'):
                ...
                tool_cli_node_parity = tool_cli_node_parity or registry.get_tool("cli_node_parity")
                tool_schema_conformance = tool_schema_conformance or registry.get_tool("schema_conformance")
                tool_error_code_usage = tool_error_code_usage or registry.get_tool("error_code_usage")
                tool_contract_compliance = tool_contract_compliance or registry.get_tool("contract_compliance")
                tool_introspection_validity = tool_introspection_validity or registry.get_tool("introspection_validity")
        # Fallback to canonical defaults if not provided
        if input_validation_tool is None:
            input_validation_tool = ToolInputValidation(
                ParityValidatorInputState, ParityValidatorOutputState, OnexFieldModel, node_id="node_parity_validator"
            )
        ...
        self.tool_cli_node_parity = tool_cli_node_parity
        self.tool_schema_conformance = tool_schema_conformance
        self.tool_error_code_usage = tool_error_code_usage
        self.tool_contract_compliance = tool_contract_compliance
        self.tool_introspection_validity = tool_introspection_validity
        ...
        if metadata_loader is None and registry is not None and hasattr(registry, 'get_tool'):
            metadata_loader = registry.get_tool('metadata_loader')
        if metadata_loader is None:
            raise RuntimeError("[NodeParityValidator] metadata_loader (ProtocolSchemaLoader) must be provided via DI/registry per ONEX standards.")
        ...
        if is_trace_mode():
            emit_log_event_sync(
                LogLevelEnum.TRACE,
                f"NodeParityValidator instantiated",
    ...
    def run(self, input_state: ParityValidatorInputState) -> ParityValidatorOutputState:
        """
        Orchestrates scenario execution for direct invocation. Accepts a validated ParityValidatorInputState model.
        ...
        # Use protocol-compliant output field tool, but always wrap in ModelParityValidatorOutputField
        output_field_kwargs = self.output_field_tool(input_state, input_state.model_dump())
        if isinstance(output_field_kwargs, dict):
            output_field = ModelParityValidatorOutputField(**output_field_kwargs)
        elif isinstance(output_field_kwargs, ModelParityValidatorOutputField):
            output_field = output_field_kwargs
        else:
            output_field = ModelParityValidatorOutputField(result=str(output_field_kwargs))
        ...
        return ParityValidatorOutputState(
            ...
            message="NodeParityValidator ran successfully.",
...
def get_introspection() -> dict:
    ...
    return NodeParityValidatorIntrospection.get_introspection_response()
