...
class RuntimeNode(ProtocolReducer):
    ...
    def run(self, input_state: RuntimeNodeInputState) -> RuntimeNodeOutputState:
        ...
        if not handler:
            return RuntimeNodeOutputState(result={"error": f"Unknown action: {action}"}, status="error")
    ...
    def initial_state(self) -> StateModel:
        ...
        """
        return RuntimeNodeInputState(version=SemVerModel(str(NodeMetadataBlock.from_file(NODE_ONEX_YAML_PATH).version)), required_field="", optional_field=None)
