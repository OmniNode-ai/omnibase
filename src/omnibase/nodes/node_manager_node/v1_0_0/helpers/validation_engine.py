...
class ValidationEngine:
    ...
    def _validate_file_content(
        ...
        try:
            ...
            if "TEMPLATE" in content and file_path.suffix in ['.py', '.yaml', '.md']:
                warnings.append(f"File contains unprocessed TEMPLATE placeholders: {file_path.name}")
            
            # Check for proper node name usage
            if file_path.suffix == '.py':
                if f"{node_name}_node" not in content and "node.py" not in file_path.name:
                    warnings.append(f"Python file may not reference node name properly: {file_path.name}")
    ...
    def _validate_node_manifest(
        ...
        try:
            ...
            required_fields = ["name", "version", "description", "entrypoint", "namespace"]
            ...
            if "name" in data and data["name"] != f"{node_name}_node":
                warnings.append(f"node.onex.yaml name field should be '{node_name}_node'")
    ...
    def _validate_state_models(
        ...
        try:
            ...
            if expected_output_class not in content:
                warnings.append(f"state.py should contain class {expected_output_class}")
