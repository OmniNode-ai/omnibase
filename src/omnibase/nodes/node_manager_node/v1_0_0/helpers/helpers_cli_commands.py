...
from omnibase.nodes.node_manager_node.v1_0_0.helpers.helpers_maintenance import NodeMaintenanceGenerator
...
def cli_fix_node_health(
    ...
    no_backup: bool = typer.Option(
        False,
        "--no-backup",
        help='Disable backup creation before making fixes'
    ),
    verbose: bool = typer.Option(
        False,
        "--verbose",
        help='Show detailed output for each node processed'
    )
) -> None:
    """
    ...
    if nodes:
        ...
        node_paths = [
            path for path in nodes_directory.iterdir()
    ...
    if dry_run:
        typer.echo(f"🔍 DRY RUN: Would perform health fixes for {len(node_paths)} nodes")
    else:
    ...
    for node_path in sorted(node_paths):
        ...
        if result.status == OnexStatus.SUCCESS:
            success_count += 1
            fixes_applied = result.metadata.get("fixes_applied", []) if result.metadata else []
            potential_fixes = result.metadata.get("potential_fixes", []) if result.metadata else []
            
            if dry_run:
                fix_count = len(potential_fixes)
                total_fixes += fix_count
                if verbose:
                    typer.echo(f"  🔍 Would apply {fix_count} fixes: {', '.join(potential_fixes)}")
            else:
                fix_count = len(fixes_applied)
                total_fixes += fix_count
                if verbose:
                    typer.echo(f"  ✅ Applied {fix_count} fixes: {', '.join(fixes_applied)}")
        else:
            error_count += 1
...
def cli_regenerate_contracts(
    ...
    if dry_run:
        typer.echo(f"🔍 DRY RUN: Would regenerate contracts for {len(node_paths)} nodes")
    else:
...
def cli_regenerate_manifests(
    ...
    if dry_run:
        typer.echo(f"🔍 DRY RUN: Would regenerate manifests for {len(node_paths)} nodes")
    else:
