...
class ToolBackendSelectionProtocol(Protocol):
    ...
    Accepts a strongly-typed KafkaEventBusConfigModel and returns a ProtocolEventBus instance.
    """
    def select_event_bus(self, config: Optional[KafkaEventBusConfigModel] = None, logger=None) -> ProtocolEventBus:
        """
        Select and instantiate the appropriate event bus backend (Kafka or InMemory) based on config.
        Args:
            config: KafkaEventBusConfigModel or None
