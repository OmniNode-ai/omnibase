...
class ToolBootstrap(ToolBootstrapProtocol):
    ...
    def bootstrap_kafka_cluster(
        ...
        emit_log_event_sync(
            LogLevelEnum.INFO, "[tool_bootstrap] Kafka bootstrap called."
        )
        admin = None
        try:
            ...
            if topics_to_create:
                try:
                    ...
                    emit_log_event_sync(
                        LogLevelEnum.INFO,
                        f"[tool_bootstrap] Created topics: {created_topics}",
                    )
                    ...
                    emit_log_event_sync(
                        LogLevelEnum.ERROR,
                        f"[tool_bootstrap] Error creating topics: {e}",
                    )
                    return ModelKafkaEventBusBootstrapResult(
                        status="error", message=f"Error creating topics: {e}"
                    )
            emit_log_event_sync(
                LogLevelEnum.INFO,
                f"[tool_bootstrap] Kafka bootstrap complete. Existing topics: {list(existing_topics)}",
            )
            return ModelKafkaEventBusBootstrapResult(
                status="ok",
                message=f"Bootstrap complete. Created: {created_topics}, Existing: {list(existing_topics)}",
            )
        except NoBrokersAvailable as e:
            emit_log_event_sync(
                LogLevelEnum.ERROR, f"[tool_bootstrap] Kafka broker not available: {e}"
            )
            return ModelKafkaEventBusBootstrapResult(
                status="error", message=f"No brokers available: {e}"
            )
        except KafkaError as e:
            emit_log_event_sync(
                LogLevelEnum.ERROR,
                f"[tool_bootstrap] Kafka error during bootstrap: {e}",
            )
            return ModelKafkaEventBusBootstrapResult(
                status="error", message=f"Kafka error: {e}"
            )
        except Exception as e:
            emit_log_event_sync(
                LogLevelEnum.ERROR,
                f"[tool_bootstrap] Unexpected error during Kafka bootstrap: {e}",
            )
            return ModelKafkaEventBusBootstrapResult(
