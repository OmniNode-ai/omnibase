...
# created_at: '2025-06-10T00:00:00.000000'
# description: Stamped by PythonHandler
# entrypoint: python://introspection
# hash: <to-be-stamped>
# last_modified_at: '2025-06-10T00:00:00.000000+00:00'
# lifecycle: active
# meta_type: tool
# metadata_version: 0.1.0
# name: introspection.py
# namespace: python://omnibase.nodes.node_kafka_event_bus.v1_0_0.introspection
# owner: OmniNode Team
...
"""
Introspection implementation for node_kafka_event_bus_node.
...
from omnibase.model.model_node_introspection import NodeCapabilityEnum, CLIArgumentModel
from omnibase.core.core_errors import OnexError

from .error_codes import NodeKafkaEventBusNodeErrorCode
from .models.state import NodeKafkaEventBusNodeInputState, NodeKafkaEventBusNodeOutputState
...
class NodeKafkaEventBusIntrospection(NodeIntrospectionMixin):
    ...
    def get_metadata_loader(cls):
        # For now, raise if not injected; can be extended for DI
        raise OnexError(NodeKafkaEventBusNodeErrorCode.MISSING_REQUIRED_PARAMETER, "NodeKafkaEventBusIntrospection.metadata_loader must be injected before use.")

    @classmethod
    def get_node_name(cls) -> str:
        return "node_kafka_event_bus_node"
    ...
    def get_node_description(cls) -> str:
        return "State contract for node_kafka_event_bus_node"
    ...
    def get_error_codes_class(cls):
        return NodeKafkaEventBusNodeErrorCode
    ...
    def handle_introspect_command(cls, event_bus=None):
        ...
        emit_log_event_sync(LogLevelEnum.DEBUG, "[Introspection] handle_introspect_command called", context={"node": cls.get_node_name()})
