...
class ModelKafkaEventBusConfig(BaseModel):
    ...
    topics: List[str] = Field(
        ..., description="List of Kafka topics to use for event bus communication"
    )
    security_protocol: Optional[str] = Field(
        default=None,
        description="Kafka security protocol (e.g., PLAINTEXT, SSL, SASL_PLAINTEXT, SASL_SSL)",
    )
    ...
    client_id: Optional[str] = Field(
        default=None, description="Kafka client ID for diagnostics"
    )
    ...
    acks: Optional[str] = Field(
        default="all",
        description="Kafka producer acknowledgment policy (e.g., 'all', '1', '0')",
    )
    enable_auto_commit: Optional[bool] = Field(
        default=True, description="Enable auto-commit for Kafka consumer"
    )
