...
def add_run_node_command(subparsers: Any) -> None:
    ...
    parser = subparsers.add_parser('run', help='Run an ONEX node with automatic version resolution')
    parser.add_argument('node_name', help='Name of the node to run (e.g., parity_validator_node, stamper_node)')
    parser.add_argument('--version', help='Specific version to run (defaults to latest)')
    parser.add_argument('--list-versions', action='store_true', help='List available versions for the specified node')
    parser.add_argument('--introspect', action='store_true', help='Show node introspection information')
    parser.add_argument('node_args', nargs=argparse.REMAINDER, help='Additional arguments to pass to the node')
...
def run_node_command(args: Any) -> int:
    ...
    if not resolved_version:
        emit_log_event(LogLevelEnum.ERROR, f"Node '{node_name}' not found or version unavailable.", context={'node_name': node_name, 'requested_version': requested_version}, node_id=_COMPONENT_NAME)
        return 1
    module_path = global_resolver.get_module_path(node_name, resolved_version)
    if not module_path:
        emit_log_event(LogLevelEnum.ERROR, f"Module path for node '{node_name}'@{resolved_version} not found.", context={'node_name': node_name, 'resolved_version': resolved_version}, node_id=_COMPONENT_NAME)
        return 1
    try:
        ...
        if args.introspect:
            if hasattr(module, 'get_introspection'):
                ...
                emit_log_event(LogLevelEnum.INFO, 'Node introspection data', context={'node_name': node_name, 'introspection': introspection}, node_id=_COMPONENT_NAME)
                print(introspection)
                return 0
            else:
                emit_log_event(LogLevelEnum.ERROR, f'Node {node_name} does not support introspection', context={'node_name': node_name}, node_id=_COMPONENT_NAME)
                return 1
        if hasattr(module, 'main'):
            ...
            emit_log_event(LogLevelEnum.ERROR, f'Node {node_name} does not have a main() function', context={'node_name': node_name}, node_id=_COMPONENT_NAME)
            return 1
    except ImportError as e:
        emit_log_event(LogLevelEnum.ERROR, f'Failed to import node {node_name}: {e}', context={'node_name': node_name, 'error': str(e)}, node_id=_COMPONENT_NAME)
        return 1
    except Exception as e:
        emit_log_event(LogLevelEnum.ERROR, f'Error running node {node_name}: {e}', context={'node_name': node_name, 'error': str(e)}, node_id=_COMPONENT_NAME)
        return 1
...
def main() -> None:
    ...
    parser.add_argument('--version', help='Specific version to run (defaults to latest)')
    parser.add_argument('--introspect', action='store_true', help='Show node introspection information')
    parser.add_argument('args', nargs=argparse.REMAINDER, help='Arguments to pass to the node')
