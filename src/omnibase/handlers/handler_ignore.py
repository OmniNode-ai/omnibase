...
class IgnoreFileHandler(ProtocolFileTypeHandler, MetadataBlockMixin):
    ...
    def extract_block(self, path: Path, content: str) ->tuple[Optional[Any],
        ...
        emit_log_event(LogLevelEnum.DEBUG,
            f'[EXTRACT] Starting extraction for {path}', node_id=
            'ignore_handler', event_bus=self._event_bus)
        block_pattern = (
        ...
        if not match:
            emit_log_event(LogLevelEnum.DEBUG,
                f'[EXTRACT] No metadata block found in {path}', node_id=
                'ignore_handler', event_bus=self._event_bus)
            return None, content
    ...
    def serialize_block(self, meta: object) ->str:
        ...
        """
        return serialize_metadata_block(meta, YAML_META_OPEN,
            YAML_META_CLOSE, comment_prefix='# ')
    ...
    def stamp(self, path: Path, content: str, **kwargs: Any) ->OnexResultModel:
        ...
        result_tuple: tuple[str, OnexResultModel
            ...
            extract_block_fn=self.extract_block, serialize_block_fn=self.
            serialize_block, model_cls=NodeMetadataBlock, context_defaults=
            context_defaults)
