...
class BaseFunctionDiscoverer(ABC):
    ...
    def can_handle_language(self, language: str) ->bool:
        ...
        return language.lower() in [lang.lower() for lang in self.
            supported_languages]

    def can_handle_extension(self, extension: str) ->bool:
        """Check if this discoverer can handle the specified file extension."""
        return extension.lower() in [ext.lower() for ext in self.
            supported_extensions]

    @abstractmethod
    def discover_functions(self, content: str, file_path: Optional[Path]=None
    ...
    def _extract_onex_marker(self, docstring: Optional[str]) ->bool:
        ...
        for line in docstring.split('\n'):
            line = line.strip()
            if line.startswith('@onex:function') or line.startswith(
                '@onex_function'):
                return True
        return False
    ...
    def discover_functions(self, content: str, file_path: Optional[Path]=None
        ...
        try:
            ...
            emit_log_event(LogLevelEnum.WARNING,
                f'Failed to parse Python content: {e}', node_id=
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                ...
                function_tool = FunctionTool(type='function', language=
                    FunctionLanguageEnum.PYTHON, line=line_number,
                    description=description, inputs=inputs, outputs=outputs,
                    error_codes=error_codes, side_effects=side_effects)
...
class JavaScriptFunctionDiscoverer(BaseFunctionDiscoverer):
        ...
        for i, line in enumerate(lines):
            for pattern in patterns:
                ...
                if match:
                    ...
                    language = (FunctionLanguageEnum.TYPESCRIPT if 
                        file_path and file_path.suffix in ['.ts', '.tsx'] else
                        FunctionLanguageEnum.JAVASCRIPT)
                    function_tool = FunctionTool(type='function', language=
                        language, line=line_number, description=description,
                        inputs=inputs, outputs=outputs, error_codes=
                        error_codes, side_effects=side_effects)
                    functions[func_name] = function_tool
        return functions
    ...
    def _extract_js_inputs(self, jsdoc: Optional[str], func_line: str) ->List[
        str]:
        ...
        for i, line in enumerate(lines):
            ...
            for pattern in patterns:
                ...
                if match:
                    ...
                    function_tool = FunctionTool(type='function', language=
                        FunctionLanguageEnum.BASH, line=line_number,
                        description=description, inputs=inputs, outputs=
                        outputs, error_codes=error_codes, side_effects=
                        side_effects)
                    functions[func_name] = function_tool
        return functions

    def _extract_bash_comments(self, lines: List[str], func_line: int
        ) ->Optional[str]:
...
class FunctionDiscoveryRegistry:
    ...
    def _register_default_discoverers(self) ->None:
        ...
        self._discoverers.extend([PythonFunctionDiscoverer(),
            JavaScriptFunctionDiscoverer(), BashFunctionDiscoverer()])
    ...
    def get_discoverer_for_language(self, language: str) ->Optional[
        BaseFunctionDiscoverer]:
    ...
    def get_discoverer_for_extension(self, extension: str) ->Optional[
        BaseFunctionDiscoverer]:
    ...
    def discover_functions_in_file(self, file_path: Path, content: str) ->Dict[
        str, FunctionTool]:
    ...
    def discover_functions_for_language(self, language: str, content: str
        ) ->Dict[str, FunctionTool]:
