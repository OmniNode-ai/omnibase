...
# hash: d21f2e5ff85a84d1835ea48b76ef1fcf4720ef4bf35d0ef13331dc078ca14f4a
# last_modified_at: '2025-05-29T14:14:00.952893+00:00'
# lifecycle: active
# meta_type: tool
# metadata_version: 0.1.0
# name: directory_traverser.py
# namespace: python://omnibase.utils.directory_traverser
# owner: OmniNode Team
...
class DirectoryTraverser(ProtocolDirectoryTraverser, ProtocolFileDiscoverySource):
    ...
    def _find_files_with_config(
        ...
        emit_log_event(
            ...
            context={
                "recursive": recursive,
                "traversal_mode": (
                    filter_config.traversal_mode.value
                    if hasattr(filter_config.traversal_mode, "value")
                    else str(filter_config.traversal_mode)
                ),
            },
            node_id=_COMPONENT_NAME,
        )
        ...
        emit_log_event(
            ...
            context={
                "matched_files": sorted(str(f) for f in all_files),
        ...
        for file_path in all_files:
            ...
            if skip_reason:
                emit_log_event(
                    LogLevelEnum.DEBUG,
                    "Skipping file",
                    context={"file_path": str(file_path), "skip_reason": skip_reason},
                    node_id=_COMPONENT_NAME,
                )
        ...
        emit_log_event(
            ...
            context={
                "eligible_files": sorted(str(f) for f in eligible_files),
                "total_count": len(eligible_files),
            },
            node_id=_COMPONENT_NAME,
        )
            ...
            if onexignore.exists():
                try:
                    ...
                    emit_log_event(
                        ...
                        context={"file_path": str(onexignore), "error": str(e)},
                        node_id=_COMPONENT_NAME,
                    )
            # Stop at repo root
            if (d / ".git").exists():
    ...
    def should_ignore(
        ...
        emit_log_event(
            ...
            context={"file_path": rel_path, "ignore_patterns": ignore_patterns},
            node_id=_COMPONENT_NAME,
        )
        ...
        if pathspec:
            ...
            emit_log_event(
                ...
                context={
                    ...
                    "result": "IGNORED" if matched else "NOT IGNORED",
                },
                node_id=_COMPONENT_NAME,
            )
            return bool(matched)
