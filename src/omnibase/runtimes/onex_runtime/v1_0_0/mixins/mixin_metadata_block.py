...
def get_onex_versions() ->dict[str, Any]:
    ...
    if _version_cache is not None:
        if not isinstance(_version_cache, dict):
            raise OnexError('_version_cache must be a dict', CoreErrorCode.
                INVALID_PARAMETER)
    ...
    for d in search_dirs:
        ...
        if candidate.exists():
            ...
            for key in ('metadata_version', 'protocol_version',
                'schema_version'):
                if key not in data:
                    raise OnexError(
            ...
            if not isinstance(data, dict):
                raise OnexError('.onexversion must load as a dict',
...
class MetadataBlockMixin:
    ...
    def extract_preserved_fields(self, prev_meta: Any) ->tuple[Optional[str
        ], Optional[str], Optional[str]]:
        if prev_meta is None:
            return None, None, None
        return prev_meta.uuid, prev_meta.created_at, prev_meta.hash

    def handle_result(self, status: str, path: Path, messages: list[str],
        metadata: dict[str, Any], diff: Optional[str]=None,
        auto_fix_applied: bool=False, fixed_files: Optional[list[str]]=None,
        failed_files: Optional[list[str]]=None) ->'OnexResultModel':
        from omnibase.model.model_onex_message_result import OnexResultModel
        return OnexResultModel(status=status, target=str(path), messages=
            messages, metadata=metadata, diff=diff, auto_fix_applied=
            auto_fix_applied, fixed_files=fixed_files or [], failed_files=
            failed_files or [])

    def update_metadata_block(self, prev_block: Optional[Any], updates:
        dict[str, Any], path: Path, model_cls: Optional[type]=None,
        context_defaults: Optional[dict[str, Any]]=None) ->Any:
        """
        ...
        if not entrypoint_type:
            ...
            entrypoint_type = Namespace.CANONICAL_SCHEME_MAP.get(ext, ext or
                'python')
        ...
        emit_log_event('DEBUG',
            ...
            , node_id=_COMPONENT_NAME, event_bus=self._event_bus)
        filtered_data = {k: v for k, v in final_data.items() if v is not
            None and k not in ['name', 'author', 'namespace', 'entrypoint']}
    ...
    def stamp_with_idempotency(self, *, path: Path, content: str, author:
        ...
        description: Optional[str]=None, extract_block_fn: Any=None,
        serialize_block_fn: Any=None, normalize_rest_fn: Any=None,
        model_cls: Any=None, context_defaults: Optional[dict[str, Any]]=None
        ...
        try:
            try:
                ...
                emit_log_event('DEBUG',
                    f'[STAMP] original content: {repr(content)}', node_id=
                    _COMPONENT_NAME, event_bus=self._event_bus)
                emit_log_event('DEBUG',
                    f'[STAMP] extracted rest: {repr(rest)}', node_id=
                    _COMPONENT_NAME, event_bus=self._event_bus)
                if not rest.strip():
                    raise RuntimeError(
            ...
            emit_log_event('DEBUG',
                ...
                node_id=_COMPONENT_NAME, event_bus=self._event_bus)
            if prev_meta is None:
                updates['created_at'] = self.get_file_creation_date(path
                    ) or datetime.now(timezone.utc).isoformat()
                updates['last_modified_at'] = datetime.now(timezone.utc
                    ).isoformat()
                new_block = self.update_metadata_block(prev_meta, updates,
                    path, model_cls, context_defaults)
                new_computed_hash = compute_metadata_hash_for_new_blocks(
                    metadata_dict=new_block.model_dump(), body=
                    normalized_rest, volatile_fields=volatile_fields,
                    metadata_serializer=serialize_block_fn,
                    body_canonicalizer=canonicalizer)
                new_block.hash = new_computed_hash
                final_block = new_block
            else:
                computed_hash = compute_idempotency_hash(metadata_model=
                    prev_meta, body=normalized_rest, volatile_fields=
                    volatile_fields, metadata_serializer=serialize_block_fn,
                    body_canonicalizer=canonicalizer)
                ...
                if not content_changed and not is_placeholder_hash:
                    ...
                    if normalized_rest:
                        ...
                        new_content = (f'{block_str}\n\n{rest_stripped}' if
                            rest_stripped else f'{block_str}\n')
                    else:
                    ...
                    return new_content, self.handle_result(status='success',
                        path=path, messages=[], metadata={'note':
                        'Stamped (idempotent)', 'hash': prev_meta.hash,
                        'content': new_content})
                else:
                    updates['last_modified_at'] = datetime.now(timezone.utc
                        ).isoformat()
                    updated_block = self.update_metadata_block(prev_meta,
                        updates, path, model_cls, context_defaults)
                    updated_hash = compute_idempotency_hash(metadata_model=
                        updated_block, body=normalized_rest,
            ...
            emit_log_event('DEBUG',
                f'[IDEMPOTENCY] Serialized block for {path}', node_id=
                _COMPONENT_NAME, event_bus=self._event_bus)
            if normalized_rest:
            ...
            emit_log_event('DEBUG',
                ...
                node_id=_COMPONENT_NAME, event_bus=self._event_bus)
            return new_content, self.handle_result(status='success', path=
                path, messages=[], metadata={'note':
                'Stamped (idempotent or updated)', 'hash': final_block.hash,
                'content': new_content})
        except Exception as e:
            emit_log_event('ERROR',
            ...
            return content, self.handle_result(status=OnexStatus.ERROR,
                path=path, messages=[OnexMessageModel(summary=
                f'Error stamping file: {str(e)}')], metadata={'note':
                f'Error: {str(e)}', 'content': content})
    ...
    def is_canonical_block(block: Any, model_cls: type) ->tuple[bool, list[str]
        ...
        block_dict = block.model_dump() if hasattr(block, 'model_dump'
            ) else dict(block)
            ...
            if k in canonical_fields:
                ...
                try:
                    ...
                    if isinstance(expected_type, type) and issubclass(
                        expected_type, BaseModel) and isinstance(v, dict):
                        v = expected_type(**v)
                except Exception:
                    pass
                if (v is not None and not MetadataBlockMixin.
                    _is_instance_of_type(v, expected_type)):
                    reasons.append(
    ...
    def _is_instance_of_type(value: Any, typ: Any) ->bool:
        ...
        if origin is typing.Union:
            return any(MetadataBlockMixin._is_instance_of_type(value, t) for
                t in typ.__args__)
    ...
    def remove_all_metadata_blocks(content: str, filetype: str) ->str:
        ...
        if not open_delim or not close_delim:
            return content
        block_pattern = (
            f'{re.escape(open_delim)}[\\s\\S]+?{re.escape(close_delim)}\\n*')
