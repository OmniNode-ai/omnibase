...
def file_io(request) -> InMemoryFileIO:
    ...
    if request.param == MOCK_CONTEXT:
        ...
        raise OnexError(
            f"Unknown file I/O context: {request.param}",
...
def handler_factory(request):
    ...
    def _create_handler(handler_class: type) -> Any:
        if request.param == MOCK_CONTEXT:
            ...
            raise OnexError(
                f"Unknown handler context: {request.param}",
                CoreErrorCode.INVALID_PARAMETER,
            )
...
def test_namespace_generation_regression(
    test_case: NamespaceTestCase, handler_factory, file_io: InMemoryFileIO
) -> None:
    """
    ...
    expected_pattern = (
        test_case.expected_namespace_pattern
        or rf"^{get_namespace_prefix()}\.[a-zA-Z0-9_\.]+$"
    )
    assert re.match(
        expected_pattern, generated_namespace
    ), f"Namespace '{generated_namespace}' should match pattern '{expected_pattern}'"

    # Verify it's NOT using the old broken patterns
    assert not generated_namespace.startswith(
        "onex.stamped."
...
@pytest.mark.parametrize(
    ...
    ids=lambda val: (
        f"{val[0].id}-overwrite_{val[1]}" if isinstance(val, tuple) else str(val)
    ),
)
def test_idempotency_preservation_with_overwrite(
    ...
    ), f"First stamping failed: {result1.messages}"
    ...
    emit_log_event(
        ...
        node_id="test_stamper_idempotency_regression",
    )
    assert metadata1.get("uuid") is not None, "UUID not found in initial metadata"
    assert (
        metadata1.get("created_at") is not None
    ), "created_at not found in initial metadata"
    ...
    ), f"Second stamping failed: {result2.messages}"
    ...
    assert (
        meta1_stripped == meta2_stripped
    ), "Non-volatile metadata fields should be identical for idempotency (overwrite={})".format(
        overwrite
    )
...
def test_uuid_preservation_bug_reproduction(
    test_case: UUIDPreservationTestCase, handler_factory, file_io: InMemoryFileIO
) -> None:
    """
    ...
    assert (
        result1.status == OnexStatus.SUCCESS
    ), f"Initial stamping failed: {result1.messages}"
    ...
    assert (
        result2.status == OnexStatus.SUCCESS
    ), f"Corruption fix stamping failed: {result2.messages}"
    ...
    assert (
        corrected_created_at == original_created_at
    ...
    if test_case.corruption_type == "hash_corruption":
        ...
        assert not corrected_namespace.startswith(
            "omnibase.wrong."
        ), "Namespace should not use wrong prefix"
        assert corrected_namespace.startswith(
            "omnibase."
        ...
        assert (
            corrected_hash != original_hash
        ), "Hash should change when namespace is corrected"
...
def test_content_change_updates_metadata(
    test_case: IdempotencyTestCase, handler_factory, file_io: InMemoryFileIO
) -> None:
    """
    ...
    assert metadata1.get("namespace") == metadata2.get(
        "namespace"
...
def test_proper_namespace_generation(handler_factory, file_io: InMemoryFileIO) -> None:
    ...
    test_cases = [
        ...
        (
            "src/omnibase/nodes/template_node/main.py",
            r"^omnibase\.nodes\.template_node\.main$",
        ),
        (
            "src/omnibase/handlers/handler_python.py",
            r"^omnibase\.handlers\.handler_python$",
        ),
    ...
    for file_path_str, expected_pattern in test_cases:
        ...
        ), f"Stamping failed for {file_path_str}: {result.messages}"
...
def test_debug_content_change_hash_calculation(
    handler_factory, file_io: InMemoryFileIO
) -> None:
    """
...
def compare_stamped_content_idempotent(content1: str, content2: str) -> bool:
    ...
    meta_block1, rest1 = extract_metadata_block_and_body(
        content1, "# === OmniNode:Metadata ===", "# === /OmniNode:Metadata ==="
    )
    meta_block2, rest2 = extract_metadata_block_and_body(
        content2, "# === OmniNode:Metadata ===", "# === /OmniNode:Metadata ==="
    )
    ...
    canon1 = CanonicalYAMLSerializer().canonicalize_metadata_block(
        meta_dict1,
    ...
    canon2 = CanonicalYAMLSerializer().canonicalize_metadata_block(
        meta_dict2,
        volatile_fields=volatile_fields,
        sort_keys=True,
        explicit_start=False,
